...
void CGuiGlfwGl3::Run () {
  ...
  GLFWwindow* window
      = glfwCreateWindow (1280, 720, "Dear ImGui GLFW+OpenGL3 MWVCMake Template", nullptr, nullptr);
  if (window == nullptr)
    return;
  ...
  triangle_shader.init (Utils::FileIO::readFile (assetsPath_ / "simple-shader.vs"),
                        Utils::FileIO::readFile (assetsPath_ / "simple-shader.fs"));
...
void CGuiGlfwGl3::runThread () {
  thread_ = std::thread (&CGuiGlfwGl3::Run, this);
}

void CGuiGlfwGl3::joinThread () {
  thread_.join ();
}
...
void render_conan_logo () {
  ...
  draw_list->AddQuadFilled (ImVec2 (x, y + 0.25 * sz), ImVec2 (x + 0.5 * sz, y + 0.5 * sz),
                            ImVec2 (x + sz, y + 0.25 * sz), ImVec2 (x + 0.5 * sz, y),
                            ImColor (col1));
  draw_list->AddQuadFilled (ImVec2 (x, y + 0.25 * sz), ImVec2 (x + 0.5 * sz, y + 0.5 * sz),
